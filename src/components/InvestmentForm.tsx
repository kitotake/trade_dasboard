...
export interface FormValues {
  name: string; ticker: string; type: string; sector: string;
  region: string; risk: string; invested: string; current: string;
  shares: string; notes: string;
}
...
export function formToInvestment(f: FormValues, id: string): Investment {
  return {
    id, name: f.name, ticker: f.ticker, type: f.type,
    sector: f.sector, region: f.region, risk: f.risk,
...
const InvestmentForm: FC<Props> = ({ form, setForm, onSave, onClose }) => {
  ...
  return (
    <>
      {/* form-grid-2col → 1col sur mobile via CSS */}
      <div className="form-grid-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <FormField label="Nom de l'actif *">
          <input aria-label="Nom de l'actif" placeholder="ex : LVMH"
            value={form.name} onChange={e => upd("name", e.target.value)} />
        </FormField>
        <FormField label="Ticker / ISIN">
          <input aria-label="Ticker ou ISIN" placeholder="ex : MC.PA"
            value={form.ticker} onChange={e => upd("ticker", e.target.value)} />
        </FormField>
        ...
        <FormField label="Secteur">
          <input aria-label="Secteur" placeholder="ex : Technologie"
            value={form.sector} onChange={e => upd("sector", e.target.value)} />
        </FormField>
        <FormField label="Région">
          <input aria-label="Région" placeholder="ex : FR, US, Europe…"
            value={form.region} onChange={e => upd("region", e.target.value)} />
        </FormField>
        ...
        <FormField label="Montant investi (€)">
          <input aria-label="Montant investi en euros" type="number" min="0" step="0.01" placeholder="0"
            value={form.invested} onChange={e => upd("invested", e.target.value)} />
        </FormField>
        <FormField label="Valeur actuelle (€)">
          <input aria-label="Valeur actuelle en euros" type="number" min="0" step="0.01" placeholder="0"
            value={form.current} onChange={e => upd("current", e.target.value)} />
        </FormField>
        <FormField label="Nombre de parts">
          <input aria-label="Nombre de parts" type="number" min="0" step="any" placeholder="0"
            value={form.shares} onChange={e => upd("shares", e.target.value)} />
        </FormField>
      </div>

      <FormField label="Notes / Commentaire">
        <textarea aria-label="Notes ou commentaires" rows={3}
          placeholder="Vos observations sur cet actif…"
          value={form.notes} onChange={e => upd("notes", e.target.value)} />
      </FormField>

      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
        <button className="btn-ghost" onClick={onClose} aria-label="Annuler">Annuler</button>
        <button
