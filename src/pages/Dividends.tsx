...
const Dividends: React.FC<DividendsProps> = ({ data, setData }) => {
  ...
  const save = () => {
    ...
    setData(d => ({...d, dividends: [...(d.dividends||[]), {...form, id:uid()}].sort((a,b) => b.date.localeCompare(a.date))}));
    ...
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:28 }}>
        ...
        <div className="card fade-up">
          ...
          <div className="kpi-value" style={{ color: dividends[0] ? "#fff" : SCSS.textMuted }}>{dividends[0] ? fmtE(dividends[0].amount) : "—"}</div>
          <div style={{ color:SCSS.textSecondary, fontSize:12 }}>{dividends[0]?.company}</div>
        </div>
        <div className="card fade-up">
          <div className="card-title">Nb versements</div>
          <div className="kpi-value" style={{ color: dividends.length ? "#fff" : SCSS.textMuted }}>{dividends.length || "0"}</div>
        </div>
      </div>
        ...
        <div className="card fade-up" style={{ marginBottom:24 }}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={chartData}>
              ...
              <Tooltip contentStyle={{ background:"#1a1d2e", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, color:"#fff", fontSize:13 }} formatter={(v: any) => [fmtE(v)]} />
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <span style={{ color:SCSS.textSecondary, fontSize:14 }}>{dividends.length} versement{dividends.length > 1 ? "s" : ""}</span>
        <button className="btn-primary" onClick={() => { setForm(EMPTY); setModal(true); }}>+ Ajouter un dividende</button>
      </div>

      <div className="card">
        {dividends.length === 0 ? (
          <EmptyState icon="💰" msg="Aucun dividende enregistré" sub="Ajoutez vos versements de dividendes, coupons, loyers…" cta="+ Ajouter" onCta={() => setModal(true)} />
        ) : (
          <table className="data-table">
            <thead><tr><th>Date</th><th>Société</th><th>Ticker</th><th>Montant</th><th>Type</th><th>Note</th><th></th></tr></thead>
            <tbody>
              {dividends.map(d => (
                <tr key={d.id}>
                  ...
                  <td><button className="btn-danger" style={{ padding:"4px 10px", fontSize:12 }} onClick={() => del(d.id)}>🗑</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {modal && (
        <Modal title="Ajouter un dividende" onClose={() => setModal(false)}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            ...
            <FormField label="Société *"><input placeholder="ex : Air Liquide" value={form.company} onChange={e => setForm(f => ({...f, company:e.target.value}))} /></FormField>
            <FormField label="Ticker"><input placeholder="ex : AI.PA" value={form.ticker} onChange={e => setForm(f => ({...f, ticker:e.target.value}))} /></FormField>
