...
const Reports: FC<ReportsProps> = ({ data }) => {
  ...
  const csvExport = () => {
    const rows = [
      ["Actif", "Ticker", "Type", "Investi", "Valeur", "Perf%", "Gain"],
      ...investments.map(i => [
        i.name,
        i.ticker ?? "",
        i.type ?? "",
        String(i.invested),
        String(i.current),
        pct(i.invested, i.current),
        String((Number(i.current) || 0) - (Number(i.invested) || 0)),
      ]),
    ];
    ...
    // BOM (\uFEFF) ensures Excel opens accented French characters correctly
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "trade-dashboard_rapport.csv";
    a.click();
    URL.revokeObjectURL(a.href);
  };

  // Pre-compute transaction totals outside JSX for readability
        ...
        {investments.length === 0 ? (
          ...
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
              {[
                ["Investi",     fmtE(totalInvested)],
                ["Valeur",      fmtE(totalCurrent)],
                ["Gain",        `${perf >= 0 ? "+" : ""}${perf}%`],
                ["Dividendes",  fmtE(totalDiv)],
              ].map(([k, v]) => (
                <div key={k} style={{ background: "rgba(255,255,255,0.04)", borderRadius: SCSS.radiusSm, padding: "14px 16px" }}>
            ...
            <table className="data-table">
              <thead>
                <tr>
                  <th>Actif</th><th>Type</th><th>Investi</th><th>Valeur</th><th>Perf. %</th><th>Gain €</th><th>Risque</th>
                </tr>
              </thead>
              <tbody>
                {investments.map(inv => {
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td>
                        <span className="badge" style={{ background: `${SCSS.accentRed}18`, color: SCSS.accentRed }}>
                          {inv.risk}
                        </span>
                      </td>
                    </tr>
      ...
      <div className="card fade-up fade-up-1">
        ...
        {transactions.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 16 }}>
            {[
              ["Total achats",  fmtE(achats)],
              ["Total ventes",  fmtE(ventes)],
              ["Total dépôts",  fmtE(depots)],
            ].map(([k, v]) => (
              <div key={k} style={{ background: "rgba(255,255,255,0.04)", borderRadius: SCSS.radiusSm, padding: "12px 14px" }}>
