...
const Portfolio: FC<PortfolioProps> = ({ data, setData }) => {
  ...
  const del      = (id: string) => setData((d: AppData) => ({ ...d, investments: d.investments.filter((i: Investment) => i.id !== id) }));

  const save = () => {
    if (!form.name) return;
    if (editing) {
      setData((d: AppData) => ({ ...d, investments: d.investments.map((i: Investment) => i.id === editing ? { ...form, id: editing } : i) }));
    } else {
      setData((d: AppData) => ({ ...d, investments: [...(d.investments || []), { ...form, id: uid() }] }));
    }
    setModal(false);
  };

  if (detail) {
    const inv = investments.find((i: Investment) => i.id === detail);
      ...
      <div>
        ...
        <div className="card">
          ...
          <div style={{ color: SCSS.textMuted, fontSize: 13, marginBottom: 20, fontFamily: SCSS.fontMono }}>
            {inv.ticker} · {inv.type} · {inv.sector} · {inv.region}
          </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
        <div style={{ color: SCSS.textSecondary, fontSize: 14 }}>
      ...
      {investments.length === 0 ? (
        <div className="card">
          <EmptyState icon="📊" msg="Aucun investissement enregistré" sub="Ajoutez vos actions, ETF, obligations..." cta="+ Ajouter" onCta={openAdd} />
        </div>
      ) : (
        <div className="card">
          <table className="data-table">
            <thead>
              <tr><th>Actif</th><th>Type</th><th>Investi</th><th>Valeur</th><th>Perf</th><th>Gain</th><th>Risque</th><th></th></tr>
            </thead>
            <tbody>
        ...
        <Modal title={editing ? "Modifier l'investissement" : "Ajouter un investissement"} onClose={() => setModal(false)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <FormField label="Nom de l'actif *">
              <input placeholder="ex : LVMH" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
            </FormField>
            <FormField label="Ticker">
              <input placeholder="ex : MC.PA" value={form.ticker} onChange={e => setForm(f => ({ ...f, ticker: e.target.value }))} />
            </FormField>
            ...
            <FormField label="Secteur">
              <input placeholder="ex : Technologie" value={form.sector} onChange={e => setForm(f => ({ ...f, sector: e.target.value }))} />
            </FormField>
            <FormField label="Région">
              <input placeholder="ex : FR, US, Europe…" value={form.region} onChange={e => setForm(f => ({ ...f, region: e.target.value }))} />
            </FormField>
            ...
            <FormField label="Montant investi (€)">
              <input type="number" placeholder="0" value={form.invested} onChange={e => setForm(f => ({ ...f, invested: e.target.value }))} />
            </FormField>
