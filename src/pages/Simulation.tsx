      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 20, alignItems: "start" }}>
        <div className="card fade-up">
          ...
          <FormField label="Capital initial (€)">
            <input type="number" placeholder="0" value={sim.initial} onChange={e => setSim(s => ({ ...s, initial: e.target.value }))} />
          </FormField>
          <FormField label="Versement mensuel (€)">
            <input type="number" placeholder="0" value={sim.monthly} onChange={e => setSim(s => ({ ...s, monthly: e.target.value }))} />
          </FormField>
          <FormField label="Rendement annuel estimé (%)">
            <input type="number" placeholder="7" value={sim.rate} onChange={e => setSim(s => ({ ...s, rate: e.target.value }))} />
          </FormField>
          <FormField label="Durée (années)">
            <input type="number" placeholder="10" value={sim.years} onChange={e => setSim(s => ({ ...s, years: e.target.value }))} />
          </FormField>
          <button className="btn-primary" style={{ width: "100%", marginTop: 8 }} onClick={run}>
            🚀 Simuler
          </button>
        </div>
        ...
        <div>
          {!result ? (
            <div className="card">
              <EmptyState icon="🔬" msg="Configurez la simulation" sub="Entrez vos paramètres et lancez la simulation" />
            </div>
          ) : (
