    ...
    <div>
      ...
      {goals.length === 0 ? (
        <div className="card">
          <EmptyState
            icon="🎯"
            msg="Aucun objectif défini"
            sub="Créez vos objectifs financiers : retraite, achat immobilier, fonds d'urgence…"
            cta="+ Créer"
            onCta={() => setModal(true)}
          />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 16 }}>
          {goals.map(g => {
            ...
            // Fix: use explicit null/empty check so current=0 is respected (not replaced by portfolio total)
            const hasCustomCurrent = g.current !== "" && g.current != null;
            const currentNum = hasCustomCurrent ? Number(g.current) : totalPortfolio;
              ...
              <div key={g.id} className="card fade-up">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                  <div>
                    ...
                    {g.deadline && (
                      <span style={{ color: SCSS.textMuted, fontSize: 12 }}>📅 Échéance : {g.deadline}</span>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button
                      ...
                      onClick={() => { setForm({ ...g }); setEditing(g.id); setModal(true); }}
                    >✏️</button>
                    <button
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: 13 }}>
                  <span style={{ color: SCSS.textSecondary }}>{fmtE(currentNum)}</span>
                ...
                <div className="progress-track">
                  <div
                    ...
                    style={{
                      width: `${pctVal}%`,
                      background: `linear-gradient(90deg, ${g.color || SCSS.accentCyan}88, ${g.color || SCSS.accentCyan})`,
                    }}
                  />
                </div>
                ...
                {g.note && (
                  <div style={{ marginTop: 10, fontSize: 12, color: SCSS.textMuted, fontStyle: "italic" }}>{g.note}</div>
                )}
              </div>
            );
          })}
      ...
      {modal && (
        <Modal title={editing ? "Modifier l'objectif" : "Nouvel objectif"} onClose={() => setModal(false)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            <FormField label="Montant actuel (€) — laisser vide = valeur du portefeuille">
              <input
                type="number"
                placeholder="0"
                value={form.current}
