...
const AiChat: React.FC<AiChatProps> = ({ data, onClose }) => {
  ...
  const send = async () => {
    ...
    const msg = chatMsg; setChatMsg(""); setTyping(true);
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [...history.filter(h=>h.role!="ai" || history.indexOf(h)>0).map(h=>({role:h.role==="ai"?"assistant":"user",content:h.text})), {role:"user",content:msg}]
        })
      });
      const d2 = await res.json();
      setHistory(h => [...h, { role:"ai", text:d2.content?.[0]?.text || "Désolé, je n'ai pas pu répondre." }]);
    } catch {
  ...
  return (
    <div style={{
      ...
      <div style={{ padding:"14px 18px", borderBottom:`1px solid ${SCSS.borderSub}`,
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px 14px", display:"flex", flexDirection:"column", gap:10 }}>
        ...
        {typing && (
          <div style={{ display:"flex", gap:5, padding:"10px 14px", background:"rgba(255,255,255,0.07)", borderRadius:"14px 14px 14px 4px", width:60 }}>
            {[0,1,2].map(i => <div key={i} style={{ width:6, height:6, borderRadius:"50%", background:SCSS.accentCyan, animation:`pulseRing 1s ${i*0.2}s infinite` }} />)}
          </div>
        )}
        <div ref={endRef} />
      </div>
      ...
      <div style={{ padding:"12px 14px", borderTop:`1px solid ${SCSS.borderSub}", display:"flex", gap:8 }}>
        <input value={chatMsg} onChange={e => setChatMsg(e.target.value)} onKeyDown={e => e.key==="Enter" && send()}
          placeholder="Posez une question…" style={{ flex:1, borderRadius:10 }} />
